...
- **PR review, end-to-end (GitHub + GitLab)** — `gortex prs` triages open pull / merge requests from the graph (`gortex prs <N>` for one PR; `--triage` / `--conflicts` / `--worktrees` / `--base` / `--format`; `gortex prs bundle`), and `gortex review [<base>|--diff] [--audience agent|human] [--post]` reviews a diff. The MCP surface mirrors it: `pr_risk` / `list_prs` / `get_pr_impact` / `triage_prs` / `conflicts_prs` / `suggest_reviewers` score and rank PRs against the graph, while `review` / `review_pack` / `post_review` / `pr_review_context` / `suggested_review_questions` / `critique_review` / `suppress_finding` drive the review itself. `internal/forge` routes on the repo's `origin` **host** before reading any credential — GitHub (incl. Enterprise, `GH_TOKEN` / `GITHUB_TOKEN`) or GitLab (incl. self-hosted, `GITLAB_TOKEN` / an existing `glab auth login`); a host nothing can recognise is declared under `forge.hosts` in the **global** config only. `gortex prs --base` and `list_prs`'s `base` accept a comma-separated list of accepted bases. See [docs/actions/README.md](docs/actions/README.md).
