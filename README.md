...
- **PR review, end to end (GitHub + GitLab)** — `gortex prs` triages open PRs / MRs (per-PR blast radius, merge-order conflicts via shared communities, AI-ranked queue, reviewer suggestions); `gortex review` emits line-anchored findings with a BLOCK/REVIEW/APPROVE verdict from a graph-grounded rulepack; MCP tools (`pr_risk`, `get_pr_impact`, `review`, `review_pack`, `post_review`, …) expose it to agents. The backend is chosen from the repo's `origin` remote — GitHub (incl. Enterprise) or GitLab (incl. self-hosted) → [docs/cli.md](docs/cli.md)
