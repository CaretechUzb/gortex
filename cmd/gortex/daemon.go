...
func runDaemonStart(cmd *cobra.Command, _ []string) error {
	...
	sweepJanitorNow, stopJanitor := startReconcileJanitor(
	...
	// readers such as `gortex repos`. That backend path is the field that
	// matters most to a CLI: `gortex repos` reads the freshness rows straight
	// out of the store file, and a daemon started with --backend-path put them
	// somewhere the platform default does not name.
	startupReporter.Serving(state.backendPath)

	// Everything below must follow Serving, not precede it: the reporter
	// publishes its OWN RuntimeState wholesale, so a field written into the
	// record before that call is overwritten rather than merged.
	//
...
// MultiIndexer.ReconcileAllCtx. interval=0 is a no-op; both returned functions
// can be called unconditionally.
//
// The sweep runs *before* the reconcile on purpose: a removed checkout's root
...
// closes rather than moves. It is a kick and not a second reconcile call
// site on purpose: one sweep body, one place to keep correct.
//
	...
	go func() {
		...
		sweep := func(trigger string) {
			...
			gcedCount, reconciled := func() (int, int) {
				...
				// The checkout lifecycle sweep replaced GCVanishedWorktrees,
				// which evicted on a single failed stat. This one resumes an
				// interrupted teardown and applies the availability and removal
				// graces, so an unreachable root is not confused with a removed
				// one.
				swept := 0
				if lifecycle != nil {
					report, err := lifecycle.Sweep(sweepCtx)
					if err != nil {
