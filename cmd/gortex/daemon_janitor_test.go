...
// These tests drive the janitor's KICK path, which is independent of the
// checkout lifecycle, so every call passes a nil *indexer.CheckoutLifecycle.
// The sweep body guards on it, and a nil lifecycle is also the live shape on a
// daemon that has not built one.
...
func TestTheReconcileJanitorSweepsOnceImmediatelyAfterWarmup(t *testing.T) {
	...
	sweepNow, stop := startReconcileJanitor(mi, nil, time.Hour, logger)
	defer stop()
...
func TestTheJanitorStillHonoursGORTEX_RECONCILE_INTERVAL_off(t *testing.T) {
	for _, raw := range []string{"0", "off"} {
		t.Run(raw, func(t *testing.T) {
			...
			logger, logs := janitorLogger()
			sweepNow, stop := startReconcileJanitor(mi, nil, reconcileInterval(), logger)
			defer stop()
...
func TestTheFirstSweepRunsAfterTheIndexersAreRegistered(t *testing.T) {
	...
	t.Run("a nil MultiIndexer disables the janitor outright", func(t *testing.T) {
		logger, logs := janitorLogger()
		sweepNow, stop := startReconcileJanitor(nil, nil, time.Hour, logger)
		defer stop()
