...
it. Seven layers cooperate:

| Layer | Source | What it emits |
|---|---|---|
| Controllers | `@http.route(...)` | one HTTP provider contract per (path, method); path lists and multi-line decorators are honoured, and `<model("res.partner"):p>` converters are normalised with the model recorded in `odoo_path_models` — including the plural `<models(...)>` form, a domain-carrying second argument, and the triple-quoted path such a domain forces. Odoo's method-less default is **GET + POST**, not Flask's GET-only. |
| Models | `models.Model` / `TransientModel` / `AbstractModel` | the class is tagged with `odoo_model` (its `_name`), `_inherit` becomes `extends`, `_inherits` becomes `composes`, a table edge follows the explicit `_table` when the class declares one and Odoo's `.`→`_` derivation otherwise (`derivation` records which), and every `fields.Many2one/One2many/Many2many` comodel becomes a `references` edge — the ER graph. Field nodes are minted here because Python emits none for class bodies. |
...
| CSV data | `<model.name>.csv` with an `id` header | one resource node per row, sharing the identity `<record id=>` mints, plus a `references` edge for every `:id` / `/id` column (to-many cells are comma-separated lists and are split). Chart-of-accounts and `ir.model.access` data is written this way almost exclusively, so without it the XML referencing those rows resolves to nothing. Gated on Odoo's own loading rule — the file must be *named after a model* — so an ordinary spreadsheet stays a metadata-only file node. |
...
| Manifest | `__manifest__.py` / `__openerp__.py` | a `KindModule` node named after the **directory** (not the manifest's human label), `depends` as module dependency edges, and `data`/`demo`/`assets` as edges to the real files. A globbed asset entry names a set rather than a file, so it mints no edge and its pattern is kept on the module node as `odoo_asset_globs`. |
| OWL client | `/** @odoo-module **/`, `odoo.define` | addon-aliased imports (`@web/core/registry`), pre-v15 `require("web.core")` matched against the name its `odoo.define(...)` declared, `registry.category("x").add(...)` providers meeting `useService("x")` consumers on one node, `patch(X.prototype, {...})` overrides, and `static template = "mod.tmpl"` binding a component to its QWeb markup. A v16 module that declares its pre-v15 name only as `/** @odoo-module alias=web.env */` is matched on that alias too, and a specifier climbing out of the addon's `static/src` root (`@web/../tests/helpers/utils`) is resolved against it rather than matched literally — that is how every JS test addresses its helpers. |
