...
| `codex`         | `~/.codex/config.toml` (`[mcp_servers.gortex]` + `SessionStart`, `UserPromptSubmit`, all-tool terminal `PreToolUse`, and Bash/`apply_patch` `PostToolUse` hooks), `~/.codex/AGENTS.md` rule block, repo `AGENTS.md` communities block | both       | https://developers.openai.com/codex/mcp                             |
...
| All-tool `PreToolUse` terminal gate | Uses one match-all managed hook so an enforceable localization `answer_ready` contract blocks every subsequent tool family, including `apply_patch`, before specialized handling. Without a terminal marker it is a local no-op; advisory completion still permits non-navigation contract operations. |
