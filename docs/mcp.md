...
| `post_review` | Post review findings as inline comments on a GitHub PR / GitLab MR — each anchored to its file + line. GitHub batches them into one review; GitLab posts one positioned discussion per comment (no review-batch concept), so a GitLab post is not atomic and a failure reports how many landed. The backend follows the repo's origin remote unless `provider` overrides it. Every body is secret-redacted before any payload is built; public / fork PRs require `confirm_public: true`; `dry_run: true` returns the would-post payloads with no network call |
