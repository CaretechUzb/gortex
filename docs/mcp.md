...
A graph-grounded pull-request review surface. GitHub (including GitHub Enterprise) and GitLab (including self-hosted) are both supported: the backend is chosen from the repo's `origin` remote **before any credential is read**, so the token the daemon needs follows that host — `GH_TOKEN` / `GITHUB_TOKEN` for GitHub, `GITLAB_TOKEN` (or an existing `glab auth login`) for GitLab. See [docs/actions/README.md](actions/README.md#providing-a-forge-token-to-the-daemon) for the resolution order and for declaring a host the router cannot recognise. The forge-data tools self-serve PR data via the daemon's own forge client, or accept caller-supplied data to skip the network; all are read-only. The review gate is AST-grounded — the deterministic correctness rulepack runs over the changeset and a graph-grounding pass drops false positives, with an opt-in LLM fold-in. The CLI exposes the same surface as `gortex prs` / `gortex review` ([cli.md](cli.md#pull-request-review)).

| Tool | Description |
|------|-------------|
| `list_prs` | List a repo's PRs with a one-shot review-state classification — a state label (DRAFT / BASE_MISMATCH / CHANGES_REQUESTED / APPROVED / STALE / READY), a normalized CI rollup (NONE / FAILURE / PENDING / SUCCESS), and merge blockers. `base` names the accepted base branch(es) that `BASE_MISMATCH` is judged against and takes a comma-separated list (`"16.0,aurora-redesign"`) for a repo with more than one live target; omit it to disable the base check. Pass `prs` to classify an already-fetched set with no network call |
...
| `post_review` | Post review findings as inline comments on a GitHub PR / GitLab MR — each anchored to its file + line. GitHub batches them into one review; GitLab posts one positioned discussion per comment (no review-batch concept), so a GitLab post is not atomic and a failure reports how many landed. The backend is derived from the repo's `origin` remote and cannot be overridden by an argument; the call is refused outright when no working tree resolves (pass `repo`), rather than answering from the daemon's own directory. Every body is secret-redacted before any payload is built; public / fork PRs require `confirm_public: true`; `dry_run: true` returns the would-post payloads with no network call |
