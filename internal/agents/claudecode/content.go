...
4. Add ` + "`analyze({kind: \"contracts\", options: {action: \"bridge\", mode: \"impact\", symbol: \"<id>\"}})`" + ` for wire-level consumers.
5. ` + "`analyze({kind: \"cross_repo\", options: {repo: \"<provider>\"}})`" + ` counts resolver-promoted relations only (calls / implements / extends). It is NOT a boundary census: framework-synthesized edges — ORM model references, JS module imports — carry no cross_repo_* mirror and are excluded, so it can under-report coupling by 10x. Treat the step-3 grouping as the coupling answer.
6. Report indexed coverage and name any untracked repositories as an explicit gap.
` + nativeMCPRules
