...
var legacySidecarShapes = map[string]legacySidecar{
	...
	// gen and content_gen are present because THIS fork's pre-view_gen
	// enrichment_state carried them: they were added by its own readiness
	// migration, well before upstream keyed the sidecars by generation. A
	// fixture without them would describe a store this fork never wrote.
	// TestSchemaV15CarriesForkOnlySidecarColumns covers the other direction —
	// a sidecar written by an upstream build, which has neither.
...
// TestSchemaV15CarriesForkOnlySidecarColumns is the other half of
// TestSchemaV14StoreGainsSidecarViewGenerationKeys: it migrates an
// enrichment_state written by an UPSTREAM build, which has neither view_gen nor
// this fork's gen / content_gen columns.
//
// It exists because the v15 rebuild used to copy a static column list taken
// from the registry. That list is wrong in both directions once one side of the
// merge carries extra columns: naming gen / content_gen fails outright here
// with "no such column", and omitting them silently DROPS this fork's readiness
// stamps on its own stores. rebuildSidecarAtBaseGeneration intersects the list
// with the table on disk instead, so both shapes migrate.
func TestSchemaV15CarriesForkOnlySidecarColumns(t *testing.T) {
	if currentSchemaVersion < 15 {
		t.Fatalf("currentSchemaVersion = %d, want >= 15", currentSchemaVersion)
	}

	path := filepath.Join(t.TempDir(), "upstream-shaped-enrichment.sqlite")
	seed, err := Open(path)
	if err != nil {
		t.Fatalf("create current store: %v", err)
	}
	...
	// Rewrite enrichment_state to the shape an upstream build would have left:
	// no view_gen, and none of this fork's readiness columns.
	withRawDB(t, path, func(db *sql.DB) {
		execDDL(t, db, `CREATE TABLE enrichment_state_upstream (
    repo_prefix  TEXT NOT NULL,
    provider     TEXT NOT NULL,
    indexed_sha  TEXT NOT NULL DEFAULT '',
    completed_at INTEGER NOT NULL DEFAULT 0,
    ...
    PRIMARY KEY (repo_prefix, provider)
		...
		execDDL(t, db, `INSERT INTO enrichment_state_upstream
  (repo_prefix, provider, indexed_sha, completed_at, coverage)
SELECT repo_prefix, provider, indexed_sha, completed_at, coverage
  FROM enrichment_state WHERE view_gen = 0`)
		execDDL(t, db, `DROP TABLE enrichment_state`)
		execDDL(t, db, `ALTER TABLE enrichment_state_upstream RENAME TO enrichment_state`)
		execDDL(t, db, `PRAGMA user_version = 14`)
	})

	// The migration must not fail on the absent columns.
	migrated, err := Open(path)
	if err != nil {
		t.Fatalf("reopen upstream-shaped store: %v", err)
	}
	t.Cleanup(func() { _ = migrated.Close() })
	if migrated.NeedsRebuild() {
		t.Fatal("re-keying an upstream-shaped sidecar must not signal a wipe/reindex")
	}

	// The row survives, at the base generation, with the fork columns defaulted
	// rather than absent.
	var viewGen, gen, contentGen int64
	var sha string
	err = migrated.db.QueryRow(`
SELECT view_gen, indexed_sha, gen, content_gen
  FROM enrichment_state WHERE repo_prefix = ? AND provider = ?`,
		sidecarSeedRepo, "gopls").Scan(&viewGen, &sha, &gen, &contentGen)
	if err != nil {
		t.Fatalf("read migrated enrichment row: %v", err)
	}
	if viewGen != 0 {
		t.Errorf("view_gen = %d, want 0: a migrated row belongs to the base corpus", viewGen)
	}
	if sha != "sha-1" {
		t.Errorf("indexed_sha = %q, want %q: the copy dropped a shared column", sha, "sha-1")
	}
	if gen != 0 || contentGen != 0 {
		t.Errorf("gen/content_gen = %d/%d, want 0/0: a column the source lacked must take its default",
			gen, contentGen)
	}
}
