...
-- The two tables below are the fork's readiness state, and they are
-- DELIBERATELY NOT in viewGenSidecars. Every sidecar in that registry is keyed
-- by view generation; these two are not, so one repo's readiness is shared
-- across every generation its checkouts route to. That is a known limitation
-- carried by the 2026-09-04 upstream merge, not an oversight: keying them
-- would mean threading a generation predicate through ~30 files of readiness
-- reads and writes, which is a change of its own and not a merge's job. It is
-- inert while a store has only the base generation 0, which is every store
-- that has not opted into a second view.
...
// gen and content_gen are the fork's readiness columns, carried through the
// 2026-09-04 upstream merge onto upstream's generation-keyed shape.
//
...
//
// Rows are ALSO the applicability model: the admission gate writes one row per
// APPLICABLE provider with gen 0 before any of them runs, so a provider that
// applies but has never completed is a visible gen-0 row rather than an absent
// one. That distinction matters because readiness takes the MINIMUM gen across
// a repo's rows: with absence meaning "not applicable", a single fresh provider
// would otherwise mask a sibling that never ran.
...
node_count, edge_count, extractor_versions`,
	...
	// skips a provider whose IndexedSHA still matches HEAD on a clean tree.
	{
		...
		// gen and content_gen are this fork's. The rebuild intersects this list
		// with the table on disk, so naming them here is documentation of the
		// canonical set, not a promise that every store carries them.
		columns: `repo_prefix, provider, indexed_sha, completed_at, coverage, gen, content_gen`,
	},
