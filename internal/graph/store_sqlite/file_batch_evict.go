...
func (s *Store) evictByPredicateResult(predicate string, arg any, scope evictScope) (nodesRemoved, edgesRemoved int, retErr error) {
	...
	// It reads the SCOPED predicate, not the bare one: an eviction bounded to
	// this generation must not report a prefix whose only rows live in another.
	touched, err := evictionTouchedPrefixesTx(ctx, tx, scoped, scopeArgs)
	if err != nil {
		return 0, 0, err
	}
