...
func generationReadProbes() []genProbe {
	return []genProbe{
		...
		{name: "SynthesizedEdgesSeq", run: func(t *testing.T, s *Store) []string {
			seq, scanErr := s.SynthesizedEdgesSeq()
			var out []string
			for e := range seq {
				out = append(out, fmt.Sprintf("synth %s->%s|%s|%s|%s",
					e.From, e.To, e.Kind, e.SynthesizedBy, e.Provenance))
			}
			if err := scanErr(); err != nil {
				t.Fatalf("SynthesizedEdgesSeq: %v", err)
			}
			return out
		}, identicalOK: "the fixture carries no edge with synthesized_by metadata, so both handles census nothing. The entry is here because this projection read the whole edges table with no generation predicate at all until the checklist caught it."},
		{name: "DanglingEdgeTargets", run: func(t *testing.T, s *Store) []string {
			return s.DanglingEdgeTargets(
				[]string{genReadRepo},
				[]graph.EdgeKind{graph.EdgeCalls},
			)
		}, identicalOK: "the fixture resolves every edge target, so nothing dangles in either generation. It is retained because the existence check it drives had to pair n.view_gen to e.view_gen — without that a node id present in the OTHER generation silently makes this generation's edge look resolved."},
...
func generationCapabilityChecklist() []capabilityCase {
	return []capabilityCase{
		...
		{iface: (*graph.CheckoutGrouped)(nil), skip: skipInMemory},
		{iface: (*graph.DanglingEdgeTargetReader)(nil), probe: "DanglingEdgeTargets"},
		{iface: (*graph.EnrichmentApplicabilityStore)(nil), skip: skipSidecar},
		{iface: (*graph.SynthesizedEdgeSequencer)(nil), probe: "SynthesizedEdgesSeq"},
