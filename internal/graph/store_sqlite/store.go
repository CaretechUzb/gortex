...
type storeCore struct {
	...
	// nothing.
	//
	// It sits on the core, not on the Store handle, because it describes the
	// tracked repositories behind the database rather than the view one handle
	// reads: two handles over different view generations are still looking at
	// the same set of worktrees. Store embeds *storeCore, so the accessors in
	// checkout_groups.go reach it unchanged.
