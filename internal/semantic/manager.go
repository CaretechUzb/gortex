...
func (m *Manager) EnrichAll(g graph.Store, roots map[string]string, opts EnrichOptions) ([]*EnrichResult, map[string]bool, error) {
	...
	// with no evidence at all (an empty or unindexed graph, and no caller-
	// supplied language set) we have not learned that nothing applies, and
	// declaring an empty set would assert it.
	if gateOnPresence {
...
//
// Defined in internal/graph because the store recognises the shape too — see
// the constant there for why the applicability prune has to.
const checkoutMarkerSeparator = graph.EnrichCheckoutMarkerSeparator
