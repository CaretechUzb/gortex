...
// own condensed line).
var sectionExploreOpener = bt(`For an explicitly named file to read/review/summarize, call §read(operation:"file", target:{file:"<path>"})§ directly; do not start localization. When the file, symbol, or evidence must be discovered, call §explore(operation:"localize")§ and obey §completion.required_action§; after §answer_ready§ quote §completion.final_response§ verbatim and make no further tool calls. For diagnosis or requested changes, call §explore(operation:"task")§, make at most one focused follow-up, then proceed to impact, edit, and test.
...
1. For an explicitly named file to read/review/summarize, call §read(operation:"file", target:{file:"<path>"})§ directly; do not start localization. When the file, symbol, or evidence must be discovered, call §explore(operation:"localize")§ and obey §completion.required_action§; after §answer_ready§, quote §completion.final_response§ verbatim and stop. For diagnosis or modification, call §explore(operation:"task")§.
