...
func (s *Server) registerCodingTools() {
	...
	s.addTool(
		mcp.NewTool("read_file",
			...
			mcp.WithBoolean("physical_evidence", mcp.Description("Opt in to disk-verified evidence for the full regular file: SHA-256, byte count, resolved path, source provenance, and whether returned content is the same buffer. The digest always covers disk bytes before windows, compression, redaction, caps, or editor overlays. Default: false.")),
			mcp.WithString("digest", mcp.Description("Digest algorithm for physical_evidence. Only sha256 is supported; defaults to sha256 when physical_evidence=true.")),
