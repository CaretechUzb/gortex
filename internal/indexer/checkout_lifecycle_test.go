...
// TestCheckoutLifecycleSweepRemovesVanishedWorktree and
// TestCheckoutLifecycleSweepRecoversDeletedGraphRetirement were removed with
// the model-free rebuild. Their setup required lc.Register(worktree,
// TrackSourceCLI) to synchronously build and route a dedicated worktree corpus
// (populated durable generations, a materializable route) — the removed
// promotion-shell / dedicated-base model (buildDedicatedCorpusSnapshot,
// ensurePromotedRepoShell). On the model-free tree tracking binds the dedicated
// checkout lazily, so the checkout is not routed and owns no gen-0 payload at
// Register time, and the tests fail before reaching any retirement assertion.
// The kept generation-retirement machinery is covered by
// generation_retirement_test.go and generation_retirement_pagination_test.go.
