...
func (mi *MultiIndexer) untrackRepoChecked(
	...
	// Stand a background workspace derivation down before draining the lane.
	// The teardown below takes the batch-transition read side and the
	// reachability topology writer, both of which that pass holds for its whole
	// run; without this an untrack issued mid-derivation waited it out
	// (measured: 19.7 minutes on a two-repo workspace). The pass is re-queued,
	// so the repo's departure is derived over afterwards.
	//
	// It sits here rather than in UntrackRepo, where this fork first put it,
	// because the stall is a property of the teardown and not of one entry
	// point: the forced and checkout-administration paths take the same two
	// locks.
