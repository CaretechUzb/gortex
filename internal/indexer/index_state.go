...
//
// ctx is only forwarded to reconcileRepoIndexState, and only on the restamp
// branch: it bounds the planner-statistics refresh that call site performs.
// The early returns take no store write lock and need none.
func (idx *Indexer) reconcileRepoIndexStateIfBehind(ctx context.Context, rootAbs string) {
	...
	idx.reconcileRepoIndexState(ctx, rootAbs)
}
