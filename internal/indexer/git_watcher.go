...
func (gw *GitWatcher) Start() error {
	...
	// two differ and the indexed commit is still reachable. Seeding from HEAD
	// declares every commit that landed while no watcher was running to be
	// already accounted for, and nothing else ever revisits them.
	seed := gw.seedSHA(initialSHA)
	gw.mu.Lock()
	gw.lastSHA = seed
	gw.loopStarted = true
	gw.mu.Unlock()
	...
	// block on a diff.
	if seed != "" && initialSHA != "" && seed != initialSHA {
		gw.logger.Info("git-watcher: catching up on commits that landed while the watcher was down",
			...
			zap.String("to", initialSHA[:min(len(initialSHA), 12)]))
