...
func (l *CheckoutLifecycle) BeginCheckoutMutation(ctx context.Context, checkoutID, expectedRoot string, expectedRouteEpoch int64) (*CheckoutMutation, error) {
	...
	if err == nil && (!found || route.State != store_sqlite.RouteActive || route.RouteEpoch != expectedRouteEpoch || route.CommitGenerationID <= 0 || route.DirtyGenerationID <= 0) {
		// An unbuilt route is not a stale one, and the difference decides
		// whether a retry can ever succeed.
		//
		// An automatic checkout's first layer is built on demand: the
		// coordinator starts inside a first-build window and defers a
		// speculative build until something says it is actually reading this
		// view (CheckoutCoordinator.Demand). Reads say so through
		// ActivateCheckout. A WRITE never did — so the first edit through an
		// exact worktree view found a route with no commit/dirty generation,
		// reported it as "the route changed, retry", and the retry found the
		// same thing forever, because nothing in the loop was ever going to
		// build the layer the edit was waiting for.
		//
		// Demand here lifts that deferral. It cannot make THIS call succeed —
		// the build is asynchronous and this path holds the build gate and the
		// cycle lock, so blocking on it here would deadlock against the very
		// cycle it just asked for — but it makes the caller's retry converge,
		// which is what the error already tells the caller to do. Demand raises
		// its wake exactly once, so an edit retried in a tight loop cannot
		// re-arm the coordinator's quiet window and starve the build (the
		// livelock ActivateCheckout documents).
		//
		// Called on the coordinator directly rather than through
		// CheckoutLifecycle.demandCheckout: c is already resolved, and was
		// already checked non-nil and rooted at expectedRoot above, so the
		// second map lookup could only find the same coordinator or a
		// different one.
		unbuilt := !found || route.CommitGenerationID <= 0 || route.DirtyGenerationID <= 0
		if unbuilt {
			c.Demand("checkout mutation")
			err = fmt.Errorf("%w: checkout view is not built yet; the build was requested, retry", ErrCheckoutMutationStale)
		} else {
