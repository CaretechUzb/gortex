...
func (mw *MultiWatcher) AddRepoContext(ctx context.Context, repoPrefix string, cfg config.WatchConfig) error {
	...
	for {
		...
		if idx := mw.multi.GetIndexer(repoPrefix); idx != nil {
			...
			// Both failure paths log. A git watcher that fails to start leaves
			// the repo reachable only through the per-file path, which cannot
			// restamp freshness — so the repo reads stale forever. Silence made
			// that indistinguishable from a watcher that was simply quiet, and
			// cost a full misdiagnosis.
			if gw, err := NewGitWatcher(meta.RootPath, idx, logger.With(zap.String("repo", repoPrefix))); err != nil {
				logger.Warn("git-watcher: init failed; ref changes will not be reconciled",
				...
				if err := gw.Start(); err == nil {
					...
					logger.Warn("git-watcher: start failed; ref changes will not be reconciled",
						zap.String("prefix", repoPrefix), zap.Error(err))
					_ = gw.Stop()
				}
			}
